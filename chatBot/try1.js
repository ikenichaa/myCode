...
db2.once('open', function() {
  console.log('Opened');
	// var tt= new Date();
	// var format = tt.toLocaleDateString()
...
// db.beacon.find(function (err, docs)
// {
// 		var x = docs;
// 		var length = x.length;
// 		var enterRoom =0;
   ...
   if(err) {
     ...
     console.log(obj);
		 ...
		 for (i = 0; i < length; i++)
		 {
			 ...
			 console.log(x[i].time.substring(0,2));
		...
		if (req.body.events[0].beacon.type==='enter')
		{
			db.beacon.find(function (err, docs)
			{
			    ...
			    var length = x.length;
					...
					console.log('enterRoom: '+enterRoom);
					console.log('leaveRoom: '+leftRoom);

					var count = enterRoom-leftRoom;
					if (count>=2)
					{
						reply(reply_token,"Please leave the room. :)")
					}
					else
					{
					...
					db.timer.insert(task, function (err, docs) {
						console.log(docs);
						res.send(docs);
					});
